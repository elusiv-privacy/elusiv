...
const RESERVED_ATTR_IDENTS: [&str; 4] = [
    ACC_ATTR,
    SYS_ATTR,
    PDA_ATTR,
    MAP_ATTR,
    ...
    if let syn::Data::Enum(e) = &ast.data {
        for (var_index, var) in e.variants.clone().iter().enumerate() {
            ...
            for (_, attr) in var.attrs.iter().enumerate() {
                ...
                sub_attrs.push(sub_attr);
                let sub_attrs: Vec<(&str, Option<&str>)> = sub_attrs.iter()
                    .map(|s| {
                ...
                match attr_name.as_str() {
                    ...
                    SYS_ATTR => {
                        ...
                        let key: TokenStream = value::<String>(&sub_attrs, "key").unwrap().parse().unwrap();
                    ...
                    PDA_ATTR => {
                        ...
                        let pda_offset: TokenStream = value(&sub_attrs, "pda_offset").unwrap_or(quote!(None));
                        ...
                        // ParentAccount?
                        let include_child_accounts = contains_key(&sub_attrs, "include_child_accounts");

                        let skip_abi = contains_key(&sub_attrs, "skip_abi");
                        if skip_abi {
                            let offset_ident: TokenStream = format!("{}_pda_offset", sub_attrs[0].0).parse().unwrap();

                            if pda_pubkey.is_some() {
                                let pubkey_ident: TokenStream = format!("{}_pda_pubkey", sub_attrs[0].0).parse().unwrap();
