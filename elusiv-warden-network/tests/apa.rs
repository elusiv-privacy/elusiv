...
async fn test_apa_inception() {
    ...
    for (i, exchange_keypair) in exchange_keypairs.iter().enumerate() {
        ...
        let start_application_instruction = ElusivWardenNetworkInstruction::start_apa_genesis_warden_application_instruction(
            warden_id,
            quote_start,
            WritableSignerAccount(warden.pubkey),
        );
        ...
        start_application_instruction.clone(),
            &[&warden.keypair],
        )
        .await;
        test.ix_fails_with_invalid_signer(start_application_instruction.clone()).await;
        ...
        let complete_application_instruction = ElusivWardenNetworkInstruction::complete_apa_genesis_warden_application_instruction(
            warden_id,
            quote_end,
            WritableSignerAccount(warden.pubkey),
        );
        test.ix_should_succeed(
            complete_application_instruction.clone(),
            &[&warden.keypair],
        ).await;
        test.ix_fails_with_invalid_signer(complete_application_instruction.clone()).await;
        ...
        if warden_id < APA_NETWORK_SIZE - 1 {
            ...
            test.ix_should_succeed(
                complete_application_instruction,
                &[&warden.keypair],
            ).await;
    ...
    for (i, exchange_keypair) in exchange_keypairs.iter().enumerate() {
        ...
        test.ix_fails_with_invalid_signer(valid_instruction.clone()).await;
...
trait IxFailsWith {
    ...
    async fn ix_fails_with_invalid_signer(
        &mut self,
        ix: Instruction,
    );
}
...
impl IxFailsWith for ElusivProgramTest {
    ...
    async fn ix_fails_with_transaction_error(
        ...
        let result = self.process_transaction_nonced(
            &[ix],
            signers
        ).await;
        ...
        let actual_error = assert_matches!(
            result,
            Err(BanksClientError::TransactionError(actual_error)) => actual_error
        );
    ...
    async fn ix_fails_with_invalid_signer(
        ...
        let err = self.ix_should_fail(ix, &[&invalid_signer])
            .await;
